...
class Auth extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->lang->load(array('auth', 'users'));
	}
	...
	public function login()
	{
		...
		if ($this->form_validation->run() == true)
		{
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
				...
				$this->activity->setAction('logged in')
							   ->save();

				//redirect them back to the home page
				$this->session->set_flashdata('message', $this->ion_auth->messages());
				if(!empty($this->session->userdata('last_page')))
				{
	...
	public function logout()
	{
		...
		$this->activity->setAction('logged out')
					   ->save();
	...
	public function activate($id)
	{
		...
		{
			// redirect them to the home page because they must be a super-admin to do this
			return show_error('You must be a super-admin to do this action.');
		}
		...
		if ($this->ion_auth->activate($id))
		{
			...
			$this->activity->setAction('Activated a :subject_link')
					   	   ->setSubject($id, 'user', '/users/view/'.$id)
						   ->save();
		...
		{
			// refresh the page and show errors
			$this->session->set_flashdata('message', $this->ion_auth->errors());
	...
	public function deactivate($id = NULL)
	{
		...
		$this->activity->setAction('Deactivated a :subject_link')
					   ->setSubject($id, 'user', '/users/view/'.$id)
					   ->save();
...
public function create_livreur()
{
        ...
        if ($this->form_validation->run() == true && $this->ion_auth->register($identity, $password, $email, $additional_data, $groups))
        {
            ...
            // redirect them back to the admin page
            $this->session->set_flashdata('message', $this->ion_auth->messages());
            redirect("Livraison/Gestion_livraison", 'refresh');
        }
        else
        {
            // display the create user form
...
/*
creat admin function
*/
public function createUserERP(){
		$this->data['title'] = $this->lang->line('create_user_heading');
			if (!$this->ion_auth->logged_in())
			{
				redirect('auth', 'refresh');
			...
			$this->form_validation->set_rules('last_name', $this->lang->line('create_user_validation_lname_label'), 'required');
			$this->form_validation->set_rules('email', $this->lang->line('create_user_validation_email_label'), 'required|valid_email|is_unique[' . $tables['users'] . '.email]');
			$this->form_validation->set_rules('phone_number', $this->lang->line('create_user_validation_phone_label'), 'trim');
			...
			if ($this->form_validation->run() == true)
			{
				...
				$groups = array($this->input->post('group')); // admin group
			}
			if ($this->form_validation->run() == true && $user_id = $this->ion_auth->register($identity, $password, $email, $additional_data, $groups))
			{
				...
				$this->activity->setAction('Created a new :subject_link')
							   ->setSubject($user->id, 'user', '/users/view/'.$user->id)
							   ->save();
				
				// create bank account infos for the user
				$data = array(
					'entity_id' => $user_id,
					'entity_type' => 'user',
				...
				$this->BankAccountInfosModel->create($data);
...
public function edit_livreur(){
		...
		// update the password if it was posted
			if ($this->input->post('password'))
			{
				...
				//$this->ion_auth->update($user->id, array('password' => $password ));
			...
			{
						$this->db->set('username', $this->input->post('identity'));
			            $this->db->where("id", $id); 
			            $this->db->update("users");  
			}
		...
		$data = array(
					...
					'model_vehicule'  => $this->input->post('model_vehicule'),
					'color_vehicule'  => $this->input->post('color_vehicule'),
					'matricule'  => $this->input->post('matriculation')		
				);
			$this->ion_auth->update($user->id, $data);
			redirect('Dashboard/View_Livreur/'.$id, 'refresh');
	...
	{
		$this->form_validation->set_rules('admin_password', $this->lang->line('change_password_validation_your_password_label'), 'required');
		...
		if ($this->form_validation->run() == false)
		{
			...
			$this->data['min_password_length'] = $this->config->item('min_password_length', 'ion_auth');
			$this->data['admin_password'] = array(
				'name' => 'admin_password',
				'id'   => 'admin_password',
				'type' => 'password',
			);
		...
		else
		{
			...
			$change = $this->ion_auth->changePasswordByAdmin($identity, $this->input->post('admin_password'), $this->input->post('new_password'));
		

			if ($change)
			{
				//if the password was successfully changed
				$this->data['message'] = $this->lang->line('password_updated_successfully');
				...
				$this->activity->setAction('Changed password for a :subject_link')
							   ->setSubject($id, 'user', '/users/view/'.$id)
							   ->save();
			}
			else
			{
				$this->data['message'] = $this->lang->line('your_password_is_incorrect');
				$this->data['value'] = FALSE;
			}
