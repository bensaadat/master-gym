...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mohamed
	 **/
	public function changePasswordByAdmin($identity, $admin_password, $new_password)
	{
		$this->trigger_events('pre_change_password');

		$this->trigger_events('extra_where');

		$query = $this->db->select('id, first_name, last_name, password, salt')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
		...
		$user = $query->row();
		$admin_password_matches = $this->hash_password_db($this->ion_auth->user()->row()->id, $admin_password);

		if ($admin_password_matches === TRUE)
		{
			// store the new password and reset the remember code so all remembered instances have to re-login
			$hashed_new_password  = $this->hash_password($new_password, $user->salt);
			$data = array(
			    'password' => $hashed_new_password,
			    'remember_code' => NULL,
			);
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, deleted_at')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				if ($user->active == 0 || $user->deleted_at != NULL)
				{
					$this->trigger_events('post_login_unsuccessful');
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select(array(
				...
				"(SELECT path 
				  FROM goerp_files
				  WHERE entity_id=goerp_users.id
				  AND entity_type='user'
				  AND description='avatar') AS profile_picture"
			));
		}
